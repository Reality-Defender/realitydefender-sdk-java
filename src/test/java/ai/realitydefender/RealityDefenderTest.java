...
class RealityDefenderTest {
  ...
  void setUp() {
    RealityDefenderConfig config =
        new RealityDefenderConfig(
            "test-api-key", "https://api.realitydefender.com", Duration.ofSeconds(30));
  ...
  void testBuilderWithValidApiKey() {
    RealityDefender client = RealityDefender.builder().apiKey("test-api-key").build();
  ...
  void testBuilderWithNullApiKey() {
    assertThrows(
        ...
        () -> {
          RealityDefender.builder().apiKey(null).build();
        });
  }
  ...
  void testBuilderWithEmptyApiKey() {
    assertThrows(
        IllegalArgumentException.class,
        () -> {
          RealityDefender.builder().apiKey("").build();
        });
  }
